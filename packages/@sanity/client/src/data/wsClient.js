...
module.exports = function createWsClient(WebSocketImpl, url, options) {
  ...
  const request = (method, data) => {
    ...
    const repliesForMessage$ = incomingMessages$.pipe(filter(message => message.id === id))
    ...
    const replies$ = repliesForMessage$.pipe(
      ...
      mergeMap(message => {
        ...
        return message.stream
          ? concat(incomingMessage, repliesForMessage$.pipe(takeWhile(msg => !msg.complete)))
          : incomingMessage
      }),
    ...
    const cancel$ = conn$.pipe(
      mergeMap(
        ws =>
          new Observable(() => () =>
            ws.readyState === WebSocket.OPEN && ws.send(formatRpcMessage(id, 'cancel'))
          )
      )
    )
